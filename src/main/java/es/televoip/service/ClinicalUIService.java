...
public class ClinicalUIService {
   ...
   private void applyFilters(VerticalLayout container) {
      ...
      List<ClinicalData> filteredData = allData.stream()
          .filter(cd -> {
              ...
              boolean matchesSearch = currentSearchTerm.isEmpty() ||
                  (cd.getTitle() != null && StringUtils.removeAccents(cd.getTitle().toLowerCase()).contains(normalizedSearchTerm)) ||
                  (cd.getDescription() != null && StringUtils.removeAccents(cd.getDescription().toLowerCase()).contains(normalizedSearchTerm));
   ...
   private Span createStatusIconWithText(String status) {
    ...
    /**
     * Asigna una prioridad numérica a cada estado clínico.
    ...
    public HorizontalLayout createPatientListItem(PatientData patient) {
        ...
        String phoneNumber = patient.getPhoneNumber().startsWith("+") ? patient.getPhoneNumber().substring(1) : patient.getPhoneNumber();
        ...
        if (patient.getData() != null && !patient.getData().isEmpty()) {
            ...
            Map<ClinicalStatus, Long> statusCounts = patient.getData().stream()
    ...
    */
    ...
    private void preserveFiltersState() {
       if (filterLayout != null) {
           // Guardar el estado actual
           currentSearchTerm = searchField.getValue();
    ...
    private void showMessage(String message) {
	    MyNotification.show(
	            message, 
	            Notification.Position.MIDDLE, NotificationVariant.LUMO_SUCCESS,
	            3000
	    );
	}
	...
	private void showMessageWarning(String message) {
	    MyNotification.showWarning(
	            message, 
	            Notification.Position.MIDDLE, NotificationVariant.LUMO_WARNING,
	            3000
	    );
	} 
