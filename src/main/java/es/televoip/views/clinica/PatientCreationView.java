...
public class PatientCreationView extends VerticalLayout implements Translatable {
	...
	private void createGrid() {
		...
		suspendedActionsColumn = grid.addComponentColumn(patient -> {
			...
			recoverButton.addThemeVariants(
	...
	private void activatePatient(PatientData patient) {
		...
		String confirmationMessage = String.format("%s %s?", i18nUtil.get("dialog.confirmActivate.message"),
				patient.getName());
		...
		Button confirmButton = new Button(i18nUtil.get("button.activate"), e -> {
			try {
				...
				confirmDialog.close();
				Notification.show(i18nUtil.get("notification.patientActivated"), 3000, Notification.Position.TOP_CENTER)
						.addThemeVariants(NotificationVariant.LUMO_SUCCESS);
			} catch (Exception ex) {
				Notification.show(i18nUtil.get("notification.errorActivating") + ": " + ex.getMessage(), 3000,
	...
	private void showPatientForm(PatientData patient) {
		...
		dialog.setHeaderTitle(patient.getId() == null ? i18nUtil.get("dialog.newPatient.title")
				: i18nUtil.get("dialog.editPatient.title"));
		...
		form.add(nameField, lastNameField, emailField, phoneField, genderField, addressField);
		...
		Button saveButton = new Button(i18nUtil.get("button.save"), e -> {
			if (savePatient(patient, nameField, lastNameField, emailField, phoneField, genderField, addressField)) {
				dialog.close();
				refreshGrid(); // Refresca la vista actual (activos o suspendidos)
			}
		});
	...
	private boolean savePatient(PatientData patient, TextField nameField, TextField lastNameField, EmailField emailField,
			TextField phoneField, Select<String> genderField, TextArea addressField) {
		...
		try {
			...
			Notification.show("Paciente guardado correctamente", 3000, Position.TOP_CENTER)
					.addThemeVariants(NotificationVariant.LUMO_SUCCESS);
			return true;

		} catch (Exception e) {
			Notification.show("Error al guardar: " + e.getMessage(), 3000, Position.TOP_CENTER)
	...
	private void showSuspendedPatients() {
		...
		toolbar.getChildren().filter(component -> component instanceof Button).map(component -> (Button) component)
				.forEach(button -> {
	...
	private void showDeleteDialog(PatientData patient) {
		...
		String confirmationMessage = String.format("%s %s?", i18nUtil.get("dialog.confirmDeletion.message"),
				patient.getName());
		Text message = new Text(confirmationMessage);
		...
		Button deleteButton = new Button(i18nUtil.get("button.delete"), event -> {
			try {
				...
				MyNotification.showError(i18nUtil.get("notification.errorDeleting") + ": " + e.getMessage(),
						Position.TOP_CENTER, 3000);
			}
		});
